...
class ReportsManager:
    ...
    def initialize_default_reports(self):
        """Initialize default reports if they don't exist."""
        default_reports = [
            {
            ...
            {
                "report_id": "member_id_cards",
                "title": "Member ID Cards",
                "description": "Generate printable ID cards for students and members",
                "category": "Students",
                "is_active": True,
                "parameters": [
                    {
                    ...
                    },
                    {
                        ...
                        "type": "checkbox",
                        "label": "Include QR Code on ID Cards",
                        "required": False,
                        "default": True
                    },
                    {
                        ...
                        "label": "Academic Year",
                        "required": True,
                        "options": ["2024-2025", "2025-2026", "2023-2024"],
                        "default": "2024-2025"
                    }
                ],
                "output_format": ["PDF"],
        ...
        for report in default_reports:
            ...
            existing_report = self.reports_collection.find_one({"report_id": report["report_id"]})
            
            if not existing_report:
                # Add creation timestamp for new report
                report["created_at"] = datetime.now()
                report["updated_at"] = datetime.now()
                
                # Insert the new report
                self.reports_collection.insert_one(report)
                print(f"Added new report: {report['title']}")
            else:
                # Update existing report while preserving creation date
                report["created_at"] = existing_report.get("created_at", datetime.now())
                report["updated_at"] = datetime.now()
                ...
                print(f"Updated existing report: {report['title']}")

        print("Report initialization completed")
