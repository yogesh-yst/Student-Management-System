...
if ([string]::IsNullOrEmpty($projectExists)) {
    ...
    Write-ColorOutput "⚠️  Please enable billing for this project in the Google Cloud Console:" $Yellow
...
if ([string]::IsNullOrEmpty($mongoSecretExists)) {
    ...
    if (![string]::IsNullOrEmpty($mongoUri)) {
        ...
        Write-ColorOutput "⚠️  Skipping MongoDB URI secret creation" $Yellow
    }
} else {
...
# Deploy Backend
Write-ColorOutput "🏗️  Building and deploying backend..." $Yellow
...
# Deploy Frontend
Write-ColorOutput "🏗️  Building and deploying frontend..." $Yellow
